<template>
  <section class="flex flex-col min-w-[520px] w-full">
          ...
          <!-- Messages -->
           <div class="text-center text-2x; py-12" v-if="pending">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 mx-auto animate-spin">
          ...
          >
            <div v-for="message in messages" class="flex items-end gap-3" :class="{'flex-row-reverse': !message.isSarah}">
              <!-- Profile Image -->
              <img
                v-if="message.isSarah"
              ...
              >
                <!-- Message Content -->
                <div v-if="message.isSarah" v-html="message.message"></div>
                <div v-else>
                 {{ message.message }}
                </div>
                <!-- Time -->
